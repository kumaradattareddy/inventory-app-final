...
export default function SalesPage() {
  ...
  // 🔧 changed from `.balance` → `.opening_balance`
  const currentBalance = Number(partyDetails?.opening_balance ?? 0);
  ...
  const customerCreateMutation = useMutation({
    ...
    onSuccess: (newParty: Party) => {
      ...
      // keep cache consistent with Party type; no synthetic `balance`
      queryClient.setQueryData(['parties'], (oldData: Party[] | undefined) => {
        const enhancedNewParty = {
          ...newParty,
          total_purchases: (newParty as any).total_purchases ?? 0,
          total_sales: (newParty as any).total_sales ?? 0,
        } as Party;
        return oldData ? [...oldData, enhancedNewParty] : [enhancedNewParty];
      });
      form.setValue('party_id', newParty.id);
    },
    onError: (error) => toast({ title: "Error", description: `Failed to create customer: ${(error as Error).message}`, variant: "destructive" })
  });
  ...
  const settlementPartyCreateMutation = useMutation({
    ...
    onError: (error) => toast({ title: "Error", description: `Failed to create recipient: ${(error as Error).message}`, variant: "destructive" })
  });
    ...
    <div className="container mx-auto py-2">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Card>
            <CardHeader><CardTitle>Customer & Bill Details</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              <FormField control={form.control} name="party_id" render={({ field }) => (
                <FormItem>
                  ...
                  <CreatableCombobox
                    ...
                    onCreate={(name) => customerCreateMutation.mutate(name)}
                    {...field}
              ...
              <div className="space-y-2">
                ...
                <Input
                  readOnly
                  value={`${new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" })
                    .format(Math.abs(currentBalance))} ${currentBalance < 0 ? 'Cr (Credit)' : 'Dr (Debit)'}`}
            ...
            <CardContent className="space-y-4">
              {fields.map((field, index) => (
                <div key={field.id} className="grid grid-cols-12 gap-2 items-start">
                  <div className="col-span-full md:col-span-5">
                    <FormField control={form.control} name={`items.${index}.product_id`} render={({ field }) => (
                      <FormItem>
                        ...
                        <SearchableCombobox options={productOptions} {...field} placeholder="Select product..." />
                        <FormMessage />
                      </FormItem>
                    )} />
                  </div>
                  ...
                  <div className="col-span-1 flex items-end h-full pt-8">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => append({ product_id: "", qty: 1, price_per_unit: 0 })}>
                ...
                <CardContent className="space-y-4">
                  ...
                  {watchedPaymentAmount > 0 && (
                    <div className="space-y-4 border-t pt-4">
                      ...
                      <FormField control={form.control} name="settlement.party_id" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Recipient Name</FormLabel>
                          <CreatableCombobox
                            options={settlementPartyOptions}
                            onCreate={(name) => settlementPartyCreateMutation.mutate(name)}
                            {...field}
                      ...
                      <FormField control={form.control} name="settlement.amount" render={({ field }) => (
                        <FormItem>
                          <FormLabel>Amount to Settle</FormLabel>
                          <FormControl><Input type="number" step="0.01" {...field} /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )}/>
