// File: app/payments/page.tsx (CORRECTED)
'use client'
...
import { Label } from "@/components/ui/label" // ✨ FIX: Added the missing Label import
...
export default function PaymentsPage() {
    ...
    const partyCreateMutation = useMutation({
        mutationFn: (partyData: { name: string, role: string }) => fetch('/api/parties', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    const handleCreateParty = () => {
        if (newPartyName) {
            partyCreateMutation.mutate({ name: newPartyName, role: newPartyRole });
        }
    };
            ...
            <Dialog open={!!newPartyName} onOpenChange={(isOpen) => !isOpen && setNewPartyName(null)}>
                <DialogContent>
                    ...
                    <div className="py-4 space-y-2">
                        <Label>Select a Role for this Contact</Label>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setNewPartyName(null)}>Cancel</Button>
                        <Button onClick={handleCreateParty} disabled={partyCreateMutation.isPending}>
